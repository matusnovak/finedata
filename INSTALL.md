...
You can compile the FineData from the source code (see below), or [use one of the automatically built releases from GitHub Releases](https://github.com/matusnovak/finedata/releases).

## Windows

```bash
# Clone the repository
git clone https://github.com/matusnovak/finedata
cd finedata
...
When distributing an application linked with FineData, these packages are not required for the users.
...
Now, you will be able to use FineData the following way:
...
If you want your executable to be portable (for example, distributing it on other systems) you might want to add `-Wl,-rpath='${ORIGIN}'` after `g++ -o example example.o`. Then, simply copy the necessary `libfinedata.so` file into the executable folder.
